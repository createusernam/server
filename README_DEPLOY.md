...
Чтобы токены пользователей не становились невалидными после каждого `docker-compose up -d`, ключи JWT должны храниться в volume. Перед первым запуском создайте каталог на хосте и выдайте права пользователю из контейнера (образ использует uid/gid 1000):

```bash
cd /opt/spacebar/server
mkdir -p jwt-keys
sudo chown -R 1000:1000 jwt-keys
```
